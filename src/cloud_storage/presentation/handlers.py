...
from fastapi import APIRouter, Depends, HTTPException, Request, Response, Form, UploadFile
from fastapi.responses import StreamingResponse
from starlette.responses import JSONResponse
from typing import Annotated

from cloud_storage.application.dto import UserRegisterData, MoveResourceDTO, UploadFileDTO
...
async def login_user_api(
        data: Annotated[UserRegisterRequest, Form()], use_case: FromDishka[LoginUserInteractor],
...
async def get_resource(
    ...
    resource = await use_case(path=path, user_id=user_id)
    resource_response = ResourceResponse(path=str(resource.parent_path),
                                         name=resource.name,
                                         type=resource.type,
                                         size=resource.size)
    return resource_response
...
async def search_resource(
    ...
    resource_responses = [ResourceResponse(path=str(res.parent_path),
                                           name=res.name,
                                           type=res.type,
                                           size=res.size) for res in resources]
